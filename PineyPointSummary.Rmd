...
C<-ggplot(noNA_onlysites, aes(Date, `wt %C`))+
  geom_vline(xintercept = as.POSIXct(as.Date("2021-07-05")), linetype=4, color="black")+ #date of tropical storm Elsa
  geom_rect(aes(ymin=-Inf, ymax=Inf, xmin=as.POSIXct(as.Date("2021-03-30")), xmax=as.POSIXct(as.Date("2021-04-09"))), fill="#00BA38", alpha=0.01)+
  geom_rect(aes(ymin=-Inf, ymax=Inf, xmin=as.POSIXct(as.Date("2021-06-13")), xmax=as.POSIXct(as.Date("2021-07-18"))), fill="red", alpha=0.01)+ #week of Jun 13 to July 18th reported as "high" k brevis from Beck et al., 2022, use dates
  geom_point()+
  geom_line()+
  facet_wrap(.~Location)+
...
write.csv(noNA_onlysites_summary, "./Tables/Temp_tables/2022_01_13_POM_summary.csv")
