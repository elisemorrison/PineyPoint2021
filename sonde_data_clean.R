...
## Patrick Norby checked on 12/19 and corrected and replaced the file
## had to add column header to the file on 12/19
...
write.csv(agglomerateddata, "SondeOutput/2022_12_19_sonde_data_agglomerated.csv")
...
agglomerateddata<-read.csv("SondeOutput/2022_12_19_sonde_data_agglomerated.csv")
...
trimmed<-agglomerateddata %>% 
  mutate(`Chlorophyll RFU` = replace(`Chlorophyll RFU`, `Chlorophyll RFU`>100 | `Chlorophyll RFU`<0, NA), 
         ...
         `fDOM QSU` = replace(`fDOM QSU`, `fDOM QSU`<0 | `fDOM QSU` >100, NA),
         ...
         `Turbidity FNU` = replace(`Turbidity FNU`, `Turbidity FNU`<0  | `Turbidity FNU`>50, NA), 
...
#remove NAs to evaluate how much data removed
chl_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`Chlorophyll ug/L`), ])  
fDOM_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`fDOM QSU`), ])
ODO_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`ODO mg/L`), ])
PE_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`BGA PE ug/L`), ])
sal_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`Sal psu`), ])
turb_removed<-nrow(trimmed) - nrow(trimmed[!is.na(trimmed$`Turbidity FNU`), ])
...
#Suppmentary plots for fDOM and turbidity
fDOM<-ggplot(summary_all_data, aes(`Date_ymd`, `fDOM QSU_mean`))+
...
supp_sonde_plot<-plot_grid(fDOM, turb, labels = c("A", "B"), align = "v", ncol=1)
